...
export const BlogCard=({
    ...
    return (
        <Link to={`/blog/${id.substring(1,id.length-1)}`}>
            <div className="p-4 border-b border-slate-200 pb-4 w-full max-w-screen-md cursor-pointer">
                <div className="flex">
                    <Avatar name={authorName} />
                    <div className="font-extralight pl-2 text-sm flex justify-center flex-col">{authorName ?? 'Unknown'}</div>
...
export function Avatar({ name, size = "small" }: { name?: string | null, size?: "small" | "big" }) {
    const initial = name && name.length ? name[0].toUpperCase() : "A";
    return (
